 ...
 * the user's full details and all their accounts in one /api/me call and stores
 * both in Zustand (auth store + account switcher). Clears them on sign-out.
 */
export function useBootstrap() {
  ...
  }, [query.data, setUser, setAccounts]);

  // /api/me always reads the live role from the DB (see getSession()), but
  // the JWT cookie itself — what middleware.ts reads for page-level routing —
